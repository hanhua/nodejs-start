...
function arrayShallowEqual(arr1: any[], arr2: any[]): boolean {
    return arr1.length == arr2.length && arr1.findIndex((x, i) => x !== arr2[i]) < 0;
}

function testES2017(obj1: Record<string,any>, obj2: Record<string,any>): boolean {
    const result = Object.fromEntries(Object.entries(obj1).map(([k,v]) => [k, v*v]));
    return shallowEqual(result, obj2);
}

function testES2018(obj1: Record<string,any>, obj2: Record<string,any>): boolean {
    return shallowEqual({c: 3, ...obj1}, obj2);
}

function testES2019(arr1: any[], arr2: any[]): boolean {
    return arrayShallowEqual(arr1.flat(2), arr2);
}

function testES2020(arr1: any[], arr2: any[]): boolean {
    return arrayShallowEqual(arr1.map(x => x ?? "other"), arr2);
}

type TestDataDTO = {
    ES2017: [Record<string,any>,Record<string,any>];
    ES2018: [Record<string,any>,Record<string,any>];
    ES2019: [any[],any[]];
    ES2020: [any[],any[]];
};
